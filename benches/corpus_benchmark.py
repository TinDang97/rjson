...
def main():
    ...
    ap.add_argument("--output-json", metavar="PATH", help="write machine-readable results to PATH")
    args = ap.parse_args()
    ...
    if args.output_json:
        meta = {
            ...
            "orjson": getattr(orjson, "__version__", "?"),
            "repeat": args.repeat,
            "cases": sorted({r["case"] for r in rows}),
        }
        with open(args.output_json, "w") as f:
            json.dump({"meta": meta, "results": rows, "geomean": geomeans}, f, indent=1)
