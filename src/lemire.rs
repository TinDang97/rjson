//! Eisel-Lemire decimal-to-binary conversion for f64.
//!
//! Adapted from the `fast-float` crate v0.2.0 (MIT OR Apache-2.0,
//! https://github.com/aldanor/fast-float-rust), itself a port of
//! https://github.com/fastfloat/fast_float. Only the f64 path of
//! `compute_float` and the 128-bit power-of-five table are vendored so the
//! parser can reuse the mantissa/exponent it already accumulated instead of
//! re-scanning the number text. Callers must fall back to a full parser when
//! this returns `None` (ambiguous rounding) or when the mantissa had more than
...
/// Compute `w * 10^q` correctly rounded to f64 (positive). Returns `None`
/// when the fast algorithm cannot decide the rounding.
#[inline]
pub fn compute_float64(q: i64, mut w: u64) -> Option<f64> {
    ...
    if power2 <= 0 {
        ...
        return Some(assemble(mantissa, power2));
    }
    if lo <= 1
